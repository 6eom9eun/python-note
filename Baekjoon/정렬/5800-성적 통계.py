...
for i in range(int(input())):
    ...
    del n[0]    #맨 앞 필요 없는 성적의 개수는 지워버림 (정렬에 방해가 됨)
    n.sort()  

    diff = []   #내림차순 바탕으로 인접한 점수들의 차들을 리스트에 삽입
    for j in range(len(n)-1):
        diff.append(n[j+1] - n[j])

    print('Class', i+1)
    print('Max', str(max(n))+',' ,'Min', str(min(n))+',', 'Largest gap', max(diff))
