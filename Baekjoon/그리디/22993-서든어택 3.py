...
폭풍전야. 블라인드에서 서든3을 비웃던, 지금도 비웃는 이들에게 반박글을 달지 않았다. 니들이 허접한지, 우리가 허접한지는 결과가 말해줄 것이다.
넥슨의 게임 개발자 준원이는 서든어택 3의 출시를 앞두고 게임 테스트를 하고 있다.
맵에는 준원이를 포함해 N명의 플레이어가 있다. 준원이의 공격력은 Ai이고, 나머지 사람들의 공격력은 A2,...,AN이다.
...
X = Y 이면, 아무 일도 일어나지 않는다. 드디어 전투가 시작되었다! 준원이는 최후의 생존자가 될 수 있을까?

입력
첫째 줄에는 준원이를 포함한 플레이어의 수 
N이 주어진다.

둘째 줄에는 각 플레이어의 공격력 
A1,...,AN 이 차례대로 공백을 사이에 두고 주어진다.
